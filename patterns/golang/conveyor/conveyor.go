...
/*
Паттерн Конвейер — шаблон, позволяющий разбить сложную задачу на несколько более простых подзадач.
При этом вывод первой подзадачи будет вводом для следующей.
Этот процесс повторяется до тех пор, пока все подзадачи не будут выполнены.
*/
func main() {

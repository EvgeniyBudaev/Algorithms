...
func compress(chars []byte) int {
	left, right := 0, 0 // Используем два указателя: left для записи в chars и j для обхода

	for right < len(chars) { // Проходим по массиву символов
		count := 0                  // Счетчик для подсчета количества повторяющихся символов
		currentChar := chars[right] // Текущий символ
		// Считаем количество повторяющихся символов
		for right < len(chars) && chars[right] == currentChar {
		...
		if count > 1 {
			...
			for _, digit := range strconv.Itoa(count) {
				chars[left] = byte(digit) // Записываем цифру в chars
				left++                    // Переходим к следующему индексу для записи
			}
		}
	}

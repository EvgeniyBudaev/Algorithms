...
var twoSum = function (numbers, target) {
    ...
    let right = numbers.length - 1; // Указатель на конец массива

    while (left < right) {
        const sum = numbers[left] + numbers[right]; // Вычисляем сумму текущих двух чисел

        if (sum === target) {
            // По условию задачи индексы должны быть 1-based, поэтому прибавляем 1
            return [left + 1, right + 1];
        } else if (sum < target) {
            // Если сумма меньше целевой, нам нужно увеличить её. 
            // Так как массив отсортирован, сдвигаем левый указатель вправо (к бо́льшим числам).
            left++;
        } else {
            // Если сумма больше целевой, нам нужно уменьшить её.
            // Сдвигаем правый указатель влево (к меньшим числам).
            right--;
        }
    }

    return [-1, -1]; // Запасной вариант, хотя по условию задачи решение существует всегда
};
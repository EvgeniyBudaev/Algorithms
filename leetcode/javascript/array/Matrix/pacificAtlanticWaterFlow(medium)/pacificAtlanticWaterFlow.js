...
высота соседней ячейки меньше или равна высоте текущей ячейки. Вода может перетекать из любой клетки, прилегающей к
океану, в океан.

Верните двумерный список координат сетки, где result[i] = [ri, ci] означает, что дождевая вода может течь из ячейки
...
var pacificAtlantic = function(heights) {
    ...
    for (let col = 0; col < cols; col++) {
        ...
        dfs(heights, rows - 1, col, Number.MIN_SAFE_INTEGER, atlantic);
    }

    for (let row = 0; row < rows; row++) {
        dfs(heights, row, 0, Number.MIN_SAFE_INTEGER, pacific);
        dfs(heights, row, cols - 1, Number.MIN_SAFE_INTEGER, atlantic);
    }

...
public class MergeSortedArray {
    public static void main(String[] args) {
        ...
        int[] arr2 = {2, 5, 6};
        merge(arr1, 3, arr2, 3);
        System.out.println(Arrays.toString(arr1)); // [1,2,2,3,5,6]
    }

    // merge объединяет два неотсортированных массива в один массив и сортирует его.
    // time: O(n+m), space: O(1)
    private static void merge(int[] nums1, int m, int[] nums2, int n) {
        int p1 = m - 1; // указатель на последний элемент в массиве nums1
        int p2 = n - 1; // указатель на последний элемент в массиве nums2

        for (int i = m + n - 1; i >= 0; i--) {
            // если все элементы в массиве nums2 уже были добавлены в массив nums1, то мы можем прекратить цикл
            if (p2 < 0) {
                break;
            }

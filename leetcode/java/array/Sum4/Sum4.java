...
public class Sum4 {
    ...
    private static List<List<Integer>> fourSum(int[] nums, int target) {
        Arrays.sort(nums); // Сортируем массив для удобства работы с двумя указателями [-2,-1,0,0,1,2]
        List<List<Integer>> result = new ArrayList<>(); // Результат
        int n = nums.length; // Длина массива

        for (int i = 0; i < n - 3; i++) { // Первый указатель
            // Пропускаем дубликаты для первого числа
            if (i > 0 && nums[i] == nums[i - 1]) {
                continue;
            }

            for (int j = i + 1; j < n - 2; j++) { // Второй указатель
                // Пропускаем дубликаты для второго числа
                if (j > i + 1 && nums[j] == nums[j - 1]) {
                    continue;
                }

                int left = j + 1; // Левый указатель
                int right = n - 1; // Правый указатель

                while (left < right) {
                    long sum = (long) nums[i] + nums[j] + nums[left] + nums[right]; // Сумма
        ...
        return result; // Возвращаем список всех уникальных четверок
    }
}

package array.MaximizeDistanceToClosestPerson;
...
public class MaximizeDistanceToClosestPerson {
    public static void main(String[] args) {
        ...
        System.out.println(maxDistToClosest(seats)); // 2
    }

    // maxDistToClosest - возвращает максимальное расстояние до ближайшего человека.
    // time: O(n), space: O(1)
    private static int maxDistToClosest(int[] seats) {
        int prev = -1;  // Предыдущее занятое сиденье
        int result = 0; // Максимальное расстояние до ближайшего человека
        ...
        for (int i = 0; i < seats.length; i++) {
            ...
            if (seats[i] == 1) {
                ...
                // Запоминаем индекс текущего занятого сиденья
                prev = i;
            }
        }
